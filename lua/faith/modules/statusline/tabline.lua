...
function M.get_file_path(self)
	...
	if self.file_path_is_too_long(self, path_from_root, half_nvim_width, seperator_padding) then
	...
	local seperator = utils.apply_padding(path_seperator, seperator_padding)
	if not window_has_different_root then
		path_breadcrumbs = path_breadcrumbs .. seperator
	end
	path_breadcrumbs = path_breadcrumbs .. utils.stl_escape(table.concat(folders, seperator, 1, #folders))

	return path_breadcrumbs .. " ", window_has_different_root
end

function M.create_path(self)
	local folder_icon = utils.highlight_str(utils.apply_padding(icons.ui.Project), self.colors.path.icon)
	local seperator = utils.highlight_str(icons.separators[active_sep]["left"], self.colors.path.seperator)
	...
	local root_dir = folder_icon
		.. utils.highlight_str(
			utils.apply_padding(utils.stl_escape(self.get_dir_root()), { right = root_padding }),
			self.colors.path.folder
		)
	local a = utils.highlight_str("%<" .. path_breadcrumbs, self.colors.path.folder)
	if diff_root then
		return root_dir
			.. seperator
			.. utils.highlight_str(
				utils.apply_padding(icons.separators[active_sep]["left"], { right = 1 }),
				self.colors.path.folder
			)
			.. a
			.. seperator
...
function M.create_tab(self, tabnr, colors, seperators)
	local tab_number = utils.highlight_str(utils.apply_padding(tostring(tabnr)), colors.number)
	local tab_dir =
		utils.highlight_str(utils.apply_padding(self.create_tab_path(self, tabnr) or "", { right = 1 }), colors.label)
	local tab_modified = utils.highlight_str(utils.apply_padding(self.get_tab_modified(tabnr), 0), colors.modified)
	...
	if self.options.tabs.equal_label_width then
		...
		if label_width > max_label_width then
			...
			display_label = utils.apply_padding(label_value, { left = label_pad - 1, right = (label_pad + offset) + 1 })
		end
	else
		display_label = label_value
	end

	local tab_label = utils.highlight_str(utils.apply_padding(display_label, { left = 1, right = 1 }), colors.label)

	local tab_close = ""
	if self.options.tabs.close_button then
		tab_close =
			utils.highlight_str(utils.apply_padding(self.create_close_button(tabnr), { right = 1 }), colors.close)
	end

	local l_seperator = utils.highlight_str(seperators.right, colors.seperator)
	local r_seperator = utils.highlight_str(seperators.left, colors.seperator)
	...
	if fn.tabpagenr("$") > 1 then
			...
			if visible_tabs == 1 then
				...
				elseif current_tab_number == #tabs then
					...
					if current_tab_number < math.ceil(#tabs * 0.5) then
						start_tab, end_tab =
							current_tab_number - (half_visible_tabs - offset), current_tab_number + half_visible_tabs
					elseif current_tab_number == math.ceil(#tabs * 0.5) then
						local offset2 = visible_tabs % 2 == 0 and offset or offset - 1
						offset = offset - 1
						start_tab, end_tab =
							current_tab_number - (half_visible_tabs - offset),
							current_tab_number + (half_visible_tabs - offset2)
					else
						start_tab, end_tab =
							current_tab_number - half_visible_tabs, current_tab_number + (half_visible_tabs - offset)
					end
				end
			end
		else
		...
		if fn.tabpagenr("$") > visible_tabs then
			tabline = tabline
				.. string.format(
					"%s",
					fn.tabpagenr() == #tabs and ""
						or "%@NextTab@" .. icons.ui.ArrowNavRight .. "%X" .. fn.tabpagenr("$") - end_tab
				)
		end
	end
